...
void initLCD(void)
{
   ...
   typedef int (*p_initlcd)(void*);
	void *handle;
	char *error;
	lcd_para.lcdnum=pconfig->lcd_num;
	lcd_para.pcsnum=pconfig->pcs_num;
	lcd_para.type=1;
	strcpy(lcd_para.server_ip,pconfig->lcd_server_ip);
	lcd_para.server_port = pconfig->lcd_server_port;
...
//	{"balance_rate",TY_INT,"100"},		//均衡速度,精度0.01
	if (strcmp(conf_tab[i].name, "firmid") == 0) {//0
	  ppconf->firm_id = val;
	 } else if (strcmp(conf_tab[i].name, "lcd_server_ip") == 0) {//4
	  sprintf(ppconf->lcd_server_ip, "%s", strval);
	 } else if (strcmp(conf_tab[i].name, "lcd_server_port") == 0) {//5
	  ppconf->lcd_server_port = val;
